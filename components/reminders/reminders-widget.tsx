...
// Colores de acento por día de la semana (para "upcoming" diferenciamos por día)
const DOW_ACCENT: Record<number, string> = {
  1: "bg-blue-400",    // lunes
  2: "bg-purple-400",  // martes
  3: "bg-[#E85D04]",   // miércoles
  4: "bg-teal-500",    // jueves
  5: "bg-amber-600",   // viernes
  6: "bg-violet-500",  // sábado
  0: "bg-gray-400",    // domingo
}
...
function getAccent(r: ReminderWithStatus): string {
  ...
  // upcoming: color por día de la semana del next_due
...
function buildWeekGroups(reminders: ReminderWithStatus[]): DayGroup[] {
  ...
  // Luego: agrupar los de esta semana (hoy a domingo) por día
  const thisWeekGroups: DayGroup[] = []
  const DAY_NAMES = ["Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado"]

  // Generar los días de hoy hasta el próximo domingo
  const daysAhead = 7 - today.getDay() // días hasta fin de semana
  for (let i = 0; i <= daysAhead; i++) {
    ...
    const items = reminders.filter(r =>
      r.next_due === dateStr && r.status !== "overdue"
    )
  ...
  if (overdue.length > 0) {
    result.push({
      key: "overdue",
      label: `Vencido${overdue.length > 1 ? "s" : ""}`,
      colorClass: STATUS_CONFIG.overdue.sepColor,
      items: overdue,
    })
  }
...
export function RemindersWidget({ reminders }: Props) {
  ...
  // Solo mostrar los de esta semana + vencidos
  const today = new Date()
  today.setHours(0, 0, 0, 0)
  ...
  // Aplanar todos los elementos en orden para distribuir en 2 columnas
  ...
  function renderItems(items: Item[]) {
    return items.map((item, i) => {
      if (item.type === "sep") {
        return (
          <DaySeparator
            key={`sep-${item.group.key}`}
            label={item.group.label}
            colorClass={item.group.colorClass}
          />
        )
      }
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/reminders"
              className="inline-flex items-center gap-1.5 rounded-lg bg-[#E85D04] px-3 py-1.5 text-xs font-medium text-white hover:opacity-90 transition-opacity"
            >
      ...
      <CardContent className="pt-1">
        ...
        {/* Link a futuros si hay más allá de esta semana */}
        {reminders.length > weekReminders.length && (
          <Link
            href="/reminders"
            className="flex items-center justify-center gap-1 mt-3 rounded-lg border border-dashed border-border py-2 text-xs text-muted-foreground hover:text-foreground hover:border-border transition-colors"
          >
